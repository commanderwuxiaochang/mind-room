...
def upload_to_google_drive(file_bytes, filename, mime_type):
    ...
    if "gas_url" in st.secrets and str(st.secrets["gas_url"]).strip() and "http" in str(st.secrets["gas_url"]):
        try:
            ...
            res = requests.post(gas_url, data=json.dumps(payload), headers=headers, timeout=25)
    ...
    # 管道 2：嘗試 GCP Service Account 機器人 API
    folder_id = None
    if "drive_folder_id" in st.secrets:
        folder_id = str(st.secrets["drive_folder_id"]).strip()
    elif "gcp_service_account" in st.secrets and "drive_folder_id" in st.secrets["gcp_service_account"]:
        folder_id = str(st.secrets["gcp_service_account"]["drive_folder_id"]).strip()
    ...
    if "gcp_service_account" in st.secrets and folder_id:
        try:
            ...
            # 加入 supportsAllDrives=true 確保可成功寫入共用資料夾
    ...
    if pwd == ADMIN_PASSWORD:
        ...
        if not booked_list:
            ...
            for idx, b in enumerate(booked_list):
                ...
                with col_b:
                    if st.button(f"🗑️ 刪除釋放此時段 (保留雲端與電腦照片)", key=f"del_{idx}"):
                        # 僅移除預約排程紀錄，不刪除雲端與本地照片 (作為永久財務備份)
                        booked_list.pop(idx)
                        save_json_file(BOOKING_DB_PATH, booked_list)
                        st.success("已成功刪除該筆預約釋放時段！對應之轉帳憑證照片已完好保留於雲端硬碟與電腦資料夾中。")
                        st.rerun()
    elif pwd:
        st.error("密碼錯誤！")
