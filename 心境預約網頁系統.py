...
# ==========================================
# 0. 系統核心設定（2026 官方最新 API 金鑰與 Google 雲端試算表）
# ==========================================
...
# ☁️ Google 雲端試算表連動設定
GOOGLE_SHEET_ID = "1lBx3HDtPZ4NOmKM4N9wXNPBUmDRLVij1nGykGpeI62k"
...
def sync_to_google_sheets(form_info):
    """雲端試算表安全備份連動機制"""
    try:
        import gspread
        ...
        key_path = os.path.normpath("d:/心境整理室/google_key.json")
        ...
        if os.path.exists(key_path):
            ...
            row = [
                ...
                form_info.get('name', ''),
                form_info.get('gender', ''),
                form_info.get('age', ''),
                f"{form_info.get('city', '')}{form_info.get('district', '')}",
                form_info.get('line_id', ''),
                form_info.get('service_type', ''),
                form_info.get('booking_date', ''),
                f"{form_info.get('booking_start', '')} ~ {form_info.get('booking_end', '')}",
                form_info.get('duration_label', ''),
                str(form_info.get('total_price', 0)),
                form_info.get('pay_note', ''),
                form_info.get('signature', '')
            ]
            ws.append_row(row)
    except:
        pass  # 即使雲端網路稍有延遲，亦確保本機 Obsidian 與 LINE 通知完美運作
...
如果您先前已預約過陪伴服務，只需在第 5 點填入您的 <strong>Line ID</strong> 並點擊「檢查歷史個資」，系統會自動載入您的基本資料喔！
    ...
    if check_click and q5.strip():
        history = load_json_file(CACHE_PATH, {}).get(q5.strip())
        if history:
    ...
    with col2:
        if st.button("送出預訂資料，等待款項確認", use_container_width=True):
            if uploaded_file is None:
                ...
                # ☁️ 同步寫入 Google 雲端試算表備份
                sync_to_google_sheets(st.session_state.form_data)
...
if admin_input == ADMIN_PASSWORD:
    ...
    st.markdown("---")
    st.markdown("## 🛠️ 心境整理室 - 笑長後台管理面板")
    st.markdown("<small style='color:gray;'>※ 僅笑長輸入密碼時可見，一般客戶看不見此區塊。</small>", unsafe_allow_html=True)
    
    current_bookings = load_json_file(BOOKING_DB_PATH, [])
    
    if not current_bookings:
        st.info("目前沒有任何預訂資料。")
    else:
        st.write(f"📊 目前系統中共有 **{len(current_bookings)}** 筆時段預訂紀錄：")
        
        try: current_bookings.sort(key=lambda x: (x['date'], x['start']))
        except: pass
        
        for idx, b in enumerate(current_bookings):
            expander_title = f"📅 {b['date']} 【{b['start']} ~ {b['end']}】 - 申請人：{b['name']}"
            ...
            with st.expander(expander_title):
                ...
                with col_info:
                    ...
                    * **定時提醒：** {'已發送' if b.get('reminder_sent') else '未發送（一小時前自動觸發）'}
                    ...
                    if st.button(f"❌ 刪除此筆預訂（釋放時段）", key=f"del_{idx}"):
                        r_name = b.get('receipt_name', '')
                        if r_name:
                            try: os.remove(os.path.normpath(os.path.join(UPLOAD_DIR, r_name)))
                            except: pass
                        
                        current_bookings.pop(idx)
                        save_json_file(BOOKING_DB_PATH, current_bookings)
                        st.success("💥 預訂已成功刪除！該時段已重新對外開放！")
                        time.sleep(1)
                        st.rerun()
                        
                with col_img:
                    receipt_file = b.get('receipt_name', '')
                    img_path = os.path.normpath(os.path.join(UPLOAD_DIR, receipt_file))
